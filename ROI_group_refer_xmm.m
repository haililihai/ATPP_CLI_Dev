...
save_untouch_nii(defnii,strcat(grouproipath,PART,'_',LR,'_roimask_thr',num2str(GROUP_THRES_REAL*100),'.nii.gz'));
...
for CL_NUM=2:MAX_CL_NUM
	%if ~exist(strcat(grouproipath,num2str(VOX_SIZE),'mm_',PART,'_',LR,'_',num2str(CL_NUM),'_',num2str(GROUP_THRES_REAL*100),'_group.nii'))
	    ...
	    defnii.img = img_f;
	    save_untouch_nii(defnii,strcat(grouproipath,num2str(VOX_SIZE),'mm_',PART,'_',LR,'_',num2str(CL_NUM),'_',num2str(GROUP_THRES_REAL*100),'_group.nii.gz'));
